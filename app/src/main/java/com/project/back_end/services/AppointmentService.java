    ...
    public ResponseEntity<Map<String, String>> updateAppointment(Appointment appointment, Long patientId) {
        try{
            ...
            if (!existingAppointment.getPatient().getId().equals(patientId)) {
                return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(Map.of("error", "Unauthorized: Patient ID does not match."));
            }
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(Map.of("error", "Appointment not updated."));
        }
    ...
    public ResponseEntity<Map<String, String>> cancelAppointment(Long id, String token) {
        ...
        if (validationResult.getStatusCode() == HttpStatus.OK) {
            ...
            try {
                ...
                return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(Map.of("error", "Failed to cancel appointment."));
            }
...
public Map<String, Object> getAppointments(String pname, LocalDate date, String token) {
    try {
       ...
       if (doctorId == null) {
        ...
        List<Appointment> appointments = appointmentRepository.findByDoctorIdAndAppointmentTimeBetween(doctorId, date.atStartOfDay(), date.atTime(23, 59, 59));

        return Map.of("appointments", appointments);
    } catch (Exception e) {
        return Map.of("error", "An error occurred while fetching appointments.");
    }
}
