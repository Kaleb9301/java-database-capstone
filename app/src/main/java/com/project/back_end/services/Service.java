...
public Map<String, Object> filterDoctor(String name, String specialty, String time) {
    if (name != null && specialty != null && time != null) {
        System.out.println("Filtering with all three parameters: " + name + ", " + specialty + ", " + time);
        System.out.println("result: " + doctorService.filterDoctorsByNameSpecilityandTime(name, specialty, time));
        ...
        return doctorService.filterDoctorsBySpecialtyAndTime(specialty, time);
    } else if (name != null) {
...
public int validateAppointment(Appointment appointment, Long doctorId) {
    Optional<Doctor> doctorOpt = doctorRepository.findById(doctorId);

    if (doctorOpt.isPresent()) {
        Doctor doctor = doctorOpt.get();
        List<String> availableTimes = doctorService.getDoctorAvailability(doctorId, appointment.getAppointmentDate());
        DateTimeFormatter timeFormatter = DateTimeFormatter.ofPattern("HH:mm:ss");
        String appointmentTimeStr = appointment.getAppointmentTime().toLocalTime().format(timeFormatter);
        for (String timeSlot : availableTimes) {
            String startTime = timeSlot.split("-")[0].trim();
            if (startTime.equals(appointmentTimeStr)) {
...
public boolean validatePatient(Patient patient) {
    Patient existingPatientByEmailOrPhone = patientRepository.findByEmailOrPhone(patient.getEmail(), patient.getPhone());
    return existingPatientByEmailOrPhone == null;
}
